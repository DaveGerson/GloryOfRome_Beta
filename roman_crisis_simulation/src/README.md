...
* **/narration**: the optional narration voice ("hear it performed", off by default). A narrator retells one committed GM narration for the ear on the intermediary prep model (`gemini-3.8-flash` at LOW thinking), and `gemini-3.8-flash-tts` voices the clean transcript. Settings offers three separate choices while the voice is on. **Narration style** is a preset reader: the built-in Dramatic Reader (the owner's PR #9 narrator, word for word, plus one fidelity line), the Acta Diurna (a composed, third-person gazette reading), or any deployed JSON. It can also be "In character…", where a character the player knows recounts the week in the first person from their name and public standing only (`narratorChoice.ts`, built on `knownRecipientOptionsForPlayer`), or one of the player's own narrators (`customNarrators.ts`, a device pref whose brief reaches the prompt only as JSON-quoted data, D41). **Voice** is the narrator's own or one of six curated voices. **Voice style** is "As written" by default, or an opt-in "Say in …:" prefix on the TTS input (`voiceStyle.ts`, which documents the caveat that the TTS model may read it aloud). `performanceScript.ts` is the guard. A retelling may reword freely; a sentence that introduces a name or a figure the narration never mentioned is cut and the rest voiced (`patchedOut`), falling back to the plain text only when more than half would go. A runaway, a smuggling bracketed direction or a mechanics leak is refused wholesale. `narrators.ts` defines narrator profiles (persona, optional closing `task`, prep model and thinking level, which may be a tuned model, and the narrator's own voice). A tuned narrator is built with the paid, local-only harness in `narration/tuning/` (`npm run narrator:tune`) and deployed by committing its JSON to `narration/narrators/` (see the README there). `narrationLog.ts` keeps every performance as text on the device (chronicle, Imperial Dispatch, private-scene lines; capped at 150, never in the save). A logged transcript is re-voiced without a second prep call, and the log opens from "Narration log" by the composer. `sceneVoice.ts` lets a private-scene NPC speak their own committed lines in a voice hashed from their id, with no prep call. `wav.ts` turns the TTS endpoint's PCM into a playable WAV (a port of the owner's Python reference). `narrationPlayer.ts` handles playback: one clip at a time, and an in-memory LRU of object URLs, keyed by narrator, voice and style, that is never persisted. The model calls live in `ai/tools/narrationVoice.ts`. The React seams are `hooks/useNarrationVoice.ts`, `useNarrationLog.ts`, `usePrivateSceneVoice.ts` and `useImperialDispatch.ts`.
