...
* **/narration**: the optional narration voice ("hear it performed", off by default). A narrator retells one committed GM narration for the ear. By default this is the senatorial partner, who recounts the week to the player in 1-2 dramatic paragraphs and makes plain what it means for them. It runs on the intermediary prep model (`gemini-3.8-flash` at LOW thinking), and `gemini-3.8-flash-tts` voices the clean transcript. `performanceScript.ts` is the guard. A retelling may reword freely, but this module deterministically refuses one that introduces a name or a figure the narration never mentioned, smuggles content through a bracketed direction, runs away in length, or leaks a hidden mechanic, and it falls back to the plain text. `narrators.ts` defines narrator profiles (persona, prep model and thinking level, which may be a tuned model, and the narrator's own voice). A tuned narrator is built with the paid, local-only harness in `narration/tuning/` (`npm run narrator:tune`) and deployed by committing its JSON to `narration/narrators/` (see the README there). A player's explicit voice choice in Settings overrides the narrator's own. `wav.ts` turns the TTS endpoint's PCM into a playable WAV (a port of the owner's Python reference). `narrationPlayer.ts` handles playback: one clip at a time, and an in-memory LRU of object URLs, keyed by narrator and voice, that is never persisted. The model calls live in `ai/tools/narrationVoice.ts`, and the React seams are `hooks/useNarrationVoice.ts` and `hooks/useImperialDispatch.ts`.
