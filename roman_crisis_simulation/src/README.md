...
* **/narration**: the optional narration voice ("hear it performed", off by default). `performanceScript.ts` is its privacy and validation guard: the dramatic narrator (gemini-3.8-flash) acts as a Roman bard recounting the scene in 1-2 powerful paragraphs full of fervor and pizzazz, while this module deterministically validates that no hidden mechanics or smuggled data leak through, preparing clean spoken prose for the TTS endpoint (`gemini-3.8-flash-tts`), falling back to plain text on any failure. `wav.ts` turns the TTS endpoint's PCM into a playable WAV (a port of the owner's Python reference). `narrationPlayer.ts` handles playback: one clip at a time, and an in-memory LRU of object URLs that is never persisted. The model calls live in `ai/tools/narrationVoice.ts`, and the React seam is `hooks/useNarrationVoice.ts`.
