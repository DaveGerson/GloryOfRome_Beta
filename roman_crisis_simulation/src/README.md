...
* **/narration**: the optional narration voice ("hear it performed", off by default). `performanceScript.ts` is its privacy guard: the director may only insert `<...>` delivery directions into one committed GM narration, and this module deterministically refuses any script that changes a spoken word or smuggles content through a direction, falling back to the plain text. `wav.ts` turns the TTS endpoint's PCM into a playable WAV (a port of the owner's Python reference). `narrationPlayer.ts` handles playback: one clip at a time, and an in-memory LRU of object URLs that is never persisted. `narrators.ts` defines narrator profiles: which prep model (the intermediary "director", `gemini-3.8-flash` at low thinking by default, or a tuned model) and which voice perform the narration. A tuned narrator is built with the paid, local-only harness in `narration/tuning/` (`npm run narrator:tune`) and deployed by committing its JSON to `narration/narrators/` (see the README there). The model calls live in `ai/tools/narrationVoice.ts`, and the React seam is `hooks/useNarrationVoice.ts`.
