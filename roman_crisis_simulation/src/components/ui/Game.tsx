...
export const DestinyCard: React.FC<{
    ...
    <button type="button" className="gor-destiny" onClick={onSelect} disabled={disabled} style={style}>
        {numeral && <span className="gor-destiny-numeral" style={{ fontFamily: 'var(--font-display)', fontSize: 10, fontWeight: 600, letterSpacing: '.32em', textTransform: 'uppercase', color: 'var(--gold-700)' }}>Destiny {numeral}</span>}
        {seal && <span aria-hidden="true" className="gor-destiny-seal" style={{ width: 46, height: 46, margin: '2px auto 2px', display: 'grid', placeItems: 'center', borderRadius: '46% 54% 52% 48% / 52% 46% 54% 48%', background: 'var(--seal-crimson)', boxShadow: 'inset 0 2px 3px rgba(255,255,255,.28), inset 0 -3px 5px rgba(0,0,0,.35), 0 2px 5px rgba(58,44,16,.35)', color: 'var(--seal-letter)', fontFamily: 'var(--font-display)', fontWeight: 700, fontSize: 19, textShadow: '0 -1px 1px rgba(0,0,0,.4)' }}>{seal}</span>}
        <span className="gor-destiny-name">{name}</span>
        <span className="gor-destiny-desc">{description}</span>
        {motto && <span className="gor-destiny-motto" style={{ fontFamily: 'var(--font-display)', fontSize: 11, fontWeight: 600, letterSpacing: '.13em', textTransform: 'uppercase', color: 'var(--text-muted)' }}>❦ {motto} ❦</span>}
