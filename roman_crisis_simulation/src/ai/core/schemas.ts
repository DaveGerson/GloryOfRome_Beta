...
const EventDeltaSchema = {
    ...
    properties: {
        ...
        key: { type: Type.STRING, description: "Identifier for what is changing. For 'relation', use 'entity_a_id:entity_b_id:attribute' (e.g., trust_level, perceived_threat) — the delta changes entity_a's perception of entity_b ONLY; if a change is mutual, emit a second delta with the ids reversed. For 'resource', use 'entity_id:resource_id' with a CANONICAL snake_case resource id: denarii (spendable coin), debt_denarii (a loan, paired with the +denarii it lends; the engine writes overdraft debt itself), investigations, deep_analyses, troops, guards, agents, legions, estates, ships, workshops, holding_<slug> (a unique asset, +1), legion_support, senatorial_support, popular_support, political_influence, legitimacy, military_might (0-100 standings), favors, blackmail_on_<entity_id> — or a new snake_case name only when none of these fits (synonyms such as gold, soldiers or spies are folded onto the canonical ids). For 'scheme' or 'faction', this is the entity_id. For region changes, use the region's name. For 'world', use 'economic_stability' or 'political_climate' — no other keys are recognized." },
        delta: { type: Type.NUMBER, description: "The numerical change to apply. For 'resource', the signed amount in that resource's own unit: a price paid is negative on the payer, a gain positive on the receiver; a 0-100 standing moves by a few points. For status, scheme, region, add_region, remove_region, and faction this is ignored. For rumors, this should be a float from 0.0 to 1.0 representing credibility." },
        reason: { type: Type.STRING, description: "A short NARRATIVE description of why the change occurred, for display only - it is never parsed to decide game state. For 'resource' types, name what was paid for or where the gain came from: a debit is a price (the bribe, the donative, the levy), a gain cites its payer, its creditor, or its victim; for a holding_<slug> it describes the thing held. For 'rumor' types, this contains the rumor text. For 'scheme', this is a JSON string of the complete, updated scheme object. For 'add_region', this is a JSON string of the new RegionState object. For 'faction', this is the entity_id of the new faction, or 'null' if they become unaligned. For 'status' types, this is still the narrative explanation (e.g. 'Struck down by an assassin's blade in the forum') - the actual status/location change for 'status' deltas MUST be set via new_status/new_location below, not inferred from this text. For 'world' types, this IS the new value itself — the short string to assign to the WorldState field named in 'key' (e.g. 'Failing', 'Openly Hostile')." },
...
export const EntitySchema = {
    ...
    properties: {
        ...
        resources: {
            ...
            description: "A record of holdings keyed by CANONICAL resource id: 'denarii' (spendable coin), 'investigations'/'deep_analyses' (intel), counted 'troops'/'guards'/'agents' (men who draw weekly wages of 8/12/15 denarii each), productive 'estates'/'ships'/'workshops' (600/250/150 denarii a week each), 0-100 standings ('legion_support', 'senatorial_support', 'popular_support', 'political_influence', 'legitimacy', 'military_might'), 'favors', 'blackmail_on_<entity_id>' (a list of strings), and 'holding_<slug>' (1) for a unique named asset. Use these ids rather than synonyms; add a thematic resource of your own only when none fits. Values are numbers, strings, or arrays of strings."
        },
...
export const CharacterCreationEntitySchema = {
    ...
    properties: {
        ...
        resources: {
            ...
            additionalProperties: { oneOf: [{ type: Type.STRING }, { type: Type.NUMBER }, { type: Type.ARRAY, items: { type: Type.STRING } }] },
            description: "Starting holdings keyed by CANONICAL resource id. Include 'denarii', 'deep_analyses' and 'investigations'; add counted 'troops'/'guards'/'agents' (weekly wages of 8/12/15 denarii each, paid by the engine) and productive 'estates'/'ships'/'workshops' (600/250/150 denarii a week each) as the backstory warrants, balanced so weekly wages do not exceed weekly yield plus a tenth of the denarii; one or two 0-100 standings ('legion_support', 'senatorial_support', 'popular_support', 'political_influence', 'legitimacy'); 'favors' if favours are owed; and one or two 'holding_<slug>' items (value 1) for unique assets that fit the backstory (e.g. 'holding_hidden_cache_of_weapons'). Be creative with a thematic resource beyond these only when none fits."
        },
        relationships: {
            type: Type.OBJECT,
