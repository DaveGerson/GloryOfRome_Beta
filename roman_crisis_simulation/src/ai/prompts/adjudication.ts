...
- RESOURCE REQUIREMENTS & EXPENDITURE: For every activity in this turn (whether the player's observable attempt, a spotlight NPC's proactive action, or a reactive NPC move), process whether or not resources are required:
    - If an activity REQUIRES resources (denarii, legion_support, deep_analyses, investigations, senatorial_support, blackmail, etc.):
        - Verify whether the acting entity actually possesses those resources in sufficient measure.
        - If the entity LACKS the required resources, the activity CANNOT succeed on those terms alone: NPCs in Rome reject bribes from the penniless, legions will not march without command/rations, and attempts to purchase or fund assets without coin will fail or be refused.
        - If the entity POSSESSES the required resources and the action succeeds or is undertaken, you MUST emit appropriate negative 'resource' deltas to deduct the expended resources (e.g. spending denarii for purchases, or risking/spending standing).
    - If an activity does NOT require resources (e.g. speechcraft, dialogue, negotiation, observation, worship, personal meetings, appeals to tradition or law):
        - Do NOT penalize the action for lack of coin or assets, and do not invent arbitrary resource deductions.
- WORLD DELTAS: When the turn's events plausibly shift the empire's macro condition, emit a 'world' delta. The 'key' MUST be 'economic_stability' or 'political_climate'; 'reason' is the new short string value for that field (e.g. 'Failing', 'Openly Hostile'). For 'economic_stability', prefer exactly one of these canonical grades, best to worst: ${ECONOMIC_STABILITY_GRADES.join(', ')}. At most one 'world' delta per field per turn. 'delta' is ignored for this type; set it to 0.
