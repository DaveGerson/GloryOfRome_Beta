...
| `imperialDispatch` | `narrationPerformance.ts::buildImperialDispatchPrompt` | prep (`GEMINI_NARRATION_PREP` at LOW thinking) | - (prose; cleaned by `cleanSpokenTranscript`, falls back to the fact summary) | - | `hooks/useImperialDispatch.ts` via `ai/tools/narrationVoice.ts::performImperialDispatch`, on demand from the side panel's Imperial Dispatch control |
