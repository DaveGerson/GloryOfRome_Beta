...
| `castVoices` | `voiceCasting.ts::buildVoiceCastingPrompt` | prep (`GEMINI_NARRATION_PREP` at LOW thinking) | `zVoiceCasting` (local to `ai/tools/voiceCasting.ts`) | `VoiceCastingSchema` (local to `ai/tools/voiceCasting.ts`; voice ids an enum of the catalog) | `hooks/useVoiceCast.ts` via `ai/tools/voiceCasting.ts::castVoices`. It runs once per campaign when the narration voice is first needed ('full': the narrator's reader, voice and note, plus every known individual). Then one small 'newcomers' call covers whoever became known uncast, and "Recast everyone" makes an explicit full call. Input: the theme and the names as JSON-quoted data (D41), the player's name and position, each known individual's name, position, epithet and entity type ONLY (`narration/voiceCast.ts::castingCandidatesFor`), the voice catalog, and the deployed readers. Validated member by member: an unknown voice or a missing member is cast by rule, and notes and rationales are sanitized. Any failure is the deterministic casting. Skipped in Mock Mode |
