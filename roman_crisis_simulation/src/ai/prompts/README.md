...
| `privateSceneVoice` | `narrationPerformance.ts::buildNarrationTtsPrompt` | tts | - (audio) | - | `hooks/usePrivateSceneVoice.ts`: a private-scene NPC's committed line (cleaned by `narration/sceneVoice.ts`), in the NPC's hashed voice, never styled, with no prep call |
