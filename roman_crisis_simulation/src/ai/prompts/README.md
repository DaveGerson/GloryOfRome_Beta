...
| `narrationPerformance` | `narrationPerformance.ts::buildNarrationPerformancePrompt` | prep (`GEMINI_NARRATION_PREP` at LOW thinking, or the narrator profile's own, which may be tuned) | - (prose; checked by `narration/performanceScript.ts::validatePerformance`, including the no-new-names/figures fidelity check; falls back on any failure) | - | `ai/tools/narrationVoice.ts`, on demand (or `auto` mode after a turn commits) for ONE committed GM narration; the chosen narrator (`narration/narrators.ts`, default the senatorial partner) retells it in at most 2 spoken paragraphs: persona first, then fixed fidelity and spoken-audio rules |
| `narrationVoice` | `narrationPerformance.ts::buildNarrationTtsPrompt` | tts (`GEMINI_TTS`, via `generateSpeech`) | - (audio) | - | `ai/tools/narrationVoice.ts`, right after `narrationPerformance`; voices the clean transcript directly via `GEMINI_TTS`; call log keeps an `[audio: N bytes, mime]` placeholder |
