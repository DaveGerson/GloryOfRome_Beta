...
4. 'difficulty': a target number from 5 (trivial) to 25 (nearly impossible) representing how hard this action is BEFORE the player's own skill or personality are factored in - judge it from the action's inherent ambition and any opposition described. A moderately ambitious action with no unusual opposition should land around 10-14. If an action requires spending resources or paying bribes/donatives that the character lacks (e.g. 0 denarii in their profile), judge the difficulty as nearly impossible (20-25) unless alternative collateral or martial coercion is explicitly leveraged. Still required (use a neutral placeholder such as 10) when is_consequential is false.
